...
import {
  ...
  Sliders, ArrowUpRight, Lock, LogOut, PhoneCall, HelpCircle, FileText, ChevronDown, CheckCircle
} from 'lucide-react';
              ...
              <div className="space-y-3">
                ...
                <div className="p-6 rounded-3xl bg-slate-900 border-2 border-indigo-500/60 shadow-2xl space-y-5 relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span className="px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 font-black text-xs border border-indigo-500/40 uppercase">
                      {pinnedCurrentDelivery.status === 'ASSIGNED' && '🟡 Go to Seller / Shop'}
                      {pinnedCurrentDelivery.status === 'ARRIVED_SELLER' && '🟠 Arrived Seller / Pick Package'}
                      {(pinnedCurrentDelivery.status === 'PICKED_UP' || pinnedCurrentDelivery.status === 'DELIVERING') && '🔵 Delivering to Customer'}
                      {pinnedCurrentDelivery.status === 'ARRIVED_CUSTOMER' && '🟣 Arrived at Customer / Verify OTP'}
                    </span>

                    <span className="text-base font-black text-emerald-400">
                      +৳{pinnedCurrentDelivery.deliveryFee || 85}
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="font-extrabold text-white text-base">
                          {pinnedCurrentDelivery.user?.name || pinnedCurrentDelivery.customerName || 'Resident Customer'}
                        </h3>
                        <p className="text-xs text-slate-400">Phone: {pinnedCurrentDelivery.user?.phone || pinnedCurrentDelivery.phone || '+880 1711-000000'}</p>
                      </div>

                      {/* Quick Actions: Call & Map Navigation */}
                      <div className="flex items-center gap-2">
                        <a
                          href={`tel:${pinnedCurrentDelivery.user?.phone || pinnedCurrentDelivery.phone || ''}`}
                          className="p-3 rounded-2xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/40 font-bold text-xs flex items-center gap-1"
                          title="Call Customer"
                        >
                        ...
                        <a
                          href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                            pinnedCurrentDelivery.address?.line1 || pinnedCurrentDelivery.address || 'Mohakhali DOHS'
                          )}`}
                          target="_blank"
                          rel="noreferrer"
                          className="p-3 rounded-2xl bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 border border-blue-500/40 font-bold text-xs flex items-center gap-1"
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs space-y-1">
                      <div className="flex items-center gap-1.5 text-slate-400 font-bold">
                        ...
                        <span className="text-slate-200 font-extrabold">Delivery Address:</span>
                      </div>
                      <p className="text-slate-300 font-medium pl-5">
                        {pinnedCurrentDelivery.address?.line1 || pinnedCurrentDelivery.address || 'House #12, Road #04, DOHS Mohakhali'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    {pinnedCurrentDelivery.status === 'ASSIGNED' && (
                      <button
                        type="button"
                        onClick={() => handleUpdateOrderStatus(pinnedCurrentDelivery.id, 'ARRIVED_SELLER')}
                        disabled={actionLoading === pinnedCurrentDelivery.id}
                        className="w-full py-4 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-sm shadow-xl flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {actionLoading === pinnedCurrentDelivery.id ? (
                          <Loader2 className="w-5 h-5 animate-spin" />
                        ) : (
                          <>
                            <Store className="w-5 h-5" />
                            <span>1. Arrived at Seller / Shop</span>
                          </>
                        )}
                      </button>
                    )}

                    {pinnedCurrentDelivery.status === 'ARRIVED_SELLER' && (
                      <button
                        type="button"
                        onClick={() => handleUpdateOrderStatus(pinnedCurrentDelivery.id, 'DELIVERING')}
                        disabled={actionLoading === pinnedCurrentDelivery.id}
                        className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black text-sm shadow-xl flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {actionLoading === pinnedCurrentDelivery.id ? (
                          <Loader2 className="w-5 h-5 animate-spin" />
                        ) : (
                          <>
                            ...
                            <span>2. Confirm Package Picked Up</span>
                          </>
                        )}
                      </button>
                    )}

                    {(pinnedCurrentDelivery.status === 'PICKED_UP' || pinnedCurrentDelivery.status === 'DELIVERING') && (
                      <button
                        type="button"
                        onClick={() => handleUpdateOrderStatus(pinnedCurrentDelivery.id, 'ARRIVED_CUSTOMER')}
                        disabled={actionLoading === pinnedCurrentDelivery.id}
                        className="w-full py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-sm shadow-xl flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {actionLoading === pinnedCurrentDelivery.id ? (
                          <Loader2 className="w-5 h-5 animate-spin" />
                        ) : (
                          <>
                            ...
                            <span>3. Arrived at Customer Location</span>
                          </>
                        )}
                      </button>
                    )}

                    {pinnedCurrentDelivery.status === 'ARRIVED_CUSTOMER' && (
                      <button
                        type="button"
                        onClick={() => handleUpdateOrderStatus(pinnedCurrentDelivery.id, 'COMPLETED')}
                        className="w-full py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black text-sm shadow-xl flex items-center justify-center gap-2 cursor-pointer"
                      >
                        <CheckCircle2 className="w-5 h-5" />
                        <span>4. Enter Customer OTP & Finish Delivery</span>
                      </button>
                    )}
                  </div>

                  {/* Expand Bottom Sheet Toggle */}
                  <button
                    type="button"
                    onClick={() => setSelectedOrderDetails(pinnedCurrentDelivery)}
                    className="w-full text-center text-xs font-bold text-slate-400 hover:text-white pt-1 flex items-center justify-center gap-1"
                  >
                    <span>View Package & Order Details</span>
