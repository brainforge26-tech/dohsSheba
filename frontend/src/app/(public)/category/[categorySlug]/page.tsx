...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    // Fetch Category info (with subcategories) and Products in parallel
    Promise.all([
      fetch(`${API}/product-categories/slug/${encodeURIComponent(categorySlug)}`).then((r) => r.json()).catch(() => null),
      fetch(`${API}/products?category=${encodeURIComponent(categorySlug)}&limit=100`).then((r) => r.json()).catch(() => null),
    ]).then(([catRes, prodRes]) => {
    ...
    <div className="py-6 px-4 max-w-7xl mx-auto space-y-6 font-sans text-slate-800">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <main className="flex-1 space-y-6 min-w-0">
          ...
          {/* ── Subcategories Grid Section (Matches Chaldal Category View) ── */}
